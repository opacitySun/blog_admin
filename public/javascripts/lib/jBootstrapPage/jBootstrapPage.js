(function($) {
    $.fn.jBootstrapPage = function(config) {
        ...
        var firstBtn, preBtn, nextBtn, lastBtn, goBtn;
        ...
        return this.each(function() {
            ...
            function init() {
				...
				if(c.pageInfo == true && c.maxPages >= 1){
					$(".pageInfo").show();
				}else{
					$(".pageInfo").hide();
				}
				...
				if(pageCount <= 1){
					...
					$this.append('<li><a class="next" href="#">下一页</a></li>');
				}
				...
				$this.find(".pageInfo").remove();
				$this.prepend('<div class="pageInfo">'+ c.selectedIndex+'/'+c.maxPages+'</div>');

				//创建跳转输入框
				var pageGo = "";
				pageGo += '<div class="pageGo">';
				pageGo += '<input type="text" />';
				pageGo += '<a href="#" page="pageGoBtn">前往</a>';
				pageGo += '</div>';
				$this.find(".pageGo").remove();
				$this.append(pageGo);
				goBtn = $this.find('.pageGo a');
            }
            ...
            function mathPrePage(currButtonNum, currPage, maxPage, showPage) {
            	...
            	if(currButtonNum != currPage && currButtonNum < middle) {
            		...
            		$this.find('li').eq(1).after(html);
            		
            		bindPages();
            	}
            }

            function mathFirstPage(currButtonNum, currPage, maxPage, showPage) {
                if(maxPage < 1) return;
                ...
                for(var index = 1; index <= pageLen; index++) {
                    pNum++;
                    html += '<li class="page" pNum="'+pNum+'"><a href="#" page="'+index+'">'+index+'</a></li>';
                }
            ...
            function onPageBtnClick($_this) {
            	...
            	if(selectedText == '下一页') {
                    ...
                    var selectNum = 1;
                    mathFirstPage(selectNum, 1, c.maxPages, c.maxPageButton);
			...
			function bindGos() {
				$this.find(".pageGo a").click(function() {
					$(this).unbind('pageClick');
					$(this).on('pageClick', function(e) {
						onPageBtnClick($(this));
					});
            ...
            function bindALL() {
            	$this.find("li.page a,li a.first,li a.last,li a.pre,li a.next,.pageGo a").each(function() {
            		if($(this).parent().hasClass('disabled')) return;
                ...
                $this.find("li.page a,li a.first,li a.last,li a.pre,li a.next,.pageGo a").click(function(e) {
                	e.preventDefault();
